...
export function renderSharePage({
  ...
  const issueMarkup = issues.length
    ? issues
        .map(
          (issue, index) => `
          <article class="issue" data-page="${escapeHtml(issue.pageId)}" data-issue-id="${escapeHtml(issue.id)}">
...
<head>
  ...
  <style>
    ...
    .preview-frame{position:relative;flex:0 0 auto;width:min(100%,402px)}
    .preview-frame iframe{display:block;width:100%;height:620px}
    .share-marker-layer{position:absolute;inset:0;pointer-events:none;z-index:2}
    .share-marker{position:absolute;width:32px;height:32px;padding:0;border:1px solid #fff;background:rgba(255,255,255,.74);color:#1d1d1f;border-radius:50%;font:700 12px/30px -apple-system,BlinkMacSystemFont,"SF Pro Text",sans-serif;text-align:center;transform:translate(-50%,-50%);box-shadow:0 8px 22px #0000002b,0 0 0 1px #00000014;backdrop-filter:blur(16px) saturate(1.4);cursor:pointer;pointer-events:auto;transition:transform .16s ease,box-shadow .16s ease}
    .share-marker.todo{border-color:#ff9f0a}.share-marker.recheck{border-color:#ffcc00}.share-marker.passed{border-color:#30d158}
    .share-marker:hover,.share-marker:focus-visible{transform:translate(-50%,-50%) scale(1.12);box-shadow:0 12px 28px #0000003b,0 0 0 4px #0071e326;outline:none}
    .share-marker::after{content:attr(data-note);position:absolute;left:calc(100% + 10px);top:50%;width:220px;padding:10px 12px;border:1px solid #ffffffb8;border-radius:14px;background:#ffffffd9;color:#1d1d1f;font:500 12px/1.45 -apple-system,BlinkMacSystemFont,"SF Pro Text",sans-serif;text-align:left;white-space:normal;opacity:0;pointer-events:none;transform:translateY(-50%) translateX(-4px);box-shadow:0 12px 30px #0000001f;backdrop-filter:blur(18px) saturate(1.25);transition:opacity .16s ease,transform .16s ease}
    .share-marker:hover::after,.share-marker:focus-visible::after{opacity:1;transform:translateY(-50%) translateX(0)}
    .issue-focus{border-color:#0071e3;box-shadow:0 0 0 3px #0071e31c}.issue{transition:border-color .2s,box-shadow .2s}
  </style>
</head>
<body>
  <main class="shell">
    ...
    <section class="layout">
      ...
      <section class="panel workspace"><div class="workspace-head"><h2 id="page-title">${escapeHtml(initialPage.name)}</h2><span id="page-size">${initialPage.width} × ${initialPage.height}</span></div><div class="preview"><div class="preview-frame" id="preview-frame"><iframe id="preview" title="H5 预览" src="${escapeHtml(initialPage.previewUrl)}"></iframe><div class="share-marker-layer" id="share-marker-layer" aria-label="预览标注"></div></div></div></section>
  ...
  <script>const DATA=${jsonForScript(payload)};const tabs=[...document.querySelectorAll('.page-tab')];const iframe=document.querySelector('#preview');const frameWrap=document.querySelector('#preview-frame');const layer=document.querySelector('#share-marker-layer');const title=document.querySelector('#page-title');const size=document.querySelector('#page-size');const issueCards=[...document.querySelectorAll('.issue')];let activePage=DATA.pages[0];let frameScroll=0;function pageIssues(pageId){return DATA.issues.filter(issue=>issue.pageId===pageId)}function syncMarkers(){const width=frameWrap.clientWidth||activePage.width;const scale=width/activePage.width;layer.style.height=iframe.clientHeight+'px';layer.replaceChildren();pageIssues(activePage.id).forEach((issue,index)=>{if(!issue.position)return;const marker=document.createElement('button');marker.type='button';marker.className='share-marker '+issue.status;marker.textContent=String(index+1);marker.dataset.note=issue.note||issue.label||'样式调整建议';marker.title=marker.dataset.note;marker.setAttribute('aria-label','查看第 '+(index+1)+' 条标注');marker.style.left=(issue.position.left*scale)+'px';marker.style.top=((issue.position.top-frameScroll)*scale)+'px';marker.addEventListener('click',()=>{const card=document.querySelector('.issue[data-issue-id="'+CSS.escape(issue.id)+'"]');card?.scrollIntoView({behavior:'smooth',block:'center'});card?.classList.add('issue-focus');setTimeout(()=>card?.classList.remove('issue-focus'),1200)});layer.append(marker)})}function bindFrameScroll(){try{frameScroll=iframe.contentWindow.scrollY||0;iframe.contentWindow.addEventListener('scroll',()=>{frameScroll=iframe.contentWindow.scrollY||0;syncMarkers()},{passive:true})}catch{frameScroll=0}syncMarkers()}function selectPage(id){activePage=DATA.pages.find(item=>item.id===id)||DATA.pages[0];frameWrap.style.width='min(100%, '+activePage.width+'px)';iframe.style.height=Math.min(activePage.height,760)+'px';iframe.src=activePage.previewUrl;title.textContent=activePage.name;size.textContent=activePage.width+' × '+activePage.height;tabs.forEach(tab=>tab.classList.toggle('active',tab.dataset.pageId===activePage.id));issueCards.forEach(item=>item.hidden=item.dataset.page!==activePage.id);frameScroll=0;syncMarkers()}iframe.addEventListener('load',bindFrameScroll);window.addEventListener('resize',syncMarkers);tabs.forEach(tab=>tab.addEventListener('click',()=>selectPage(tab.dataset.pageId)));selectPage(DATA.pages[0].id);</script>
</body></html>`;
}
