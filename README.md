...
上传 ZIP，或直接上传一个资源已内联的 HTML。直接上传 HTML 适合技术临时给你的单页测试版本，站点会自动把它归档成一个新版本，你可以继续做气泡标注、走查和分享。

如果 HTML 依赖同目录的图片、CSS、JS 或字体，请把 HTML 和这些资源一起打成 ZIP；这样预览才能完整保留。

也可以在上传弹窗填写 HTML 链接。站点会抓取链接返回的 HTML 并归档；链接页面依赖旁边资源时仍建议使用 ZIP。

### 二次走查技术版本

技术把线上测试 HTML 发回来后，在主项目侧栏点击“二次走查”，从这里导入 HTML 链接、单个 HTML 或 ZIP。站点会自动创建“主项目 · 二次走查”独立空间；后续技术版本继续归档在这个空间中，主项目的设计预览和版本选择不会被替换。二次走查版本仍支持页面预览、气泡标注、走查对勾、反馈导出和分享。
...
优化命令只会替换体积更小的 PNG，不会降低分辨率或改变图片内容；适合对像素一致性要求严格的交付。

如果需要 TinyPNG 类的明显减重效果，可使用视觉有损的 128 色调色板模式。该模式需要本机安装 Pillow，会保留尺寸和透明 PNG 格式，但会量化颜色：

```bash
python3 -m pip install Pillow
npm run optimize:png:visual -- <交付目录> [颜色数]
npm run pack -- <交付目录> <输出ZIP> [handoff.json]
```

默认使用 128 色；需要更小体积时可传入 `96` 或 `64`，需要更保守时可传入 `192` 或 `256`。视觉压缩前建议保留原始交付包；活动主视觉、半透明光效和文字切图应先在预览中检查。
